...
displayImg(page) : page는 현재 페이지 번호를 나타냄
    호출될 때 마다 페이지를 전환하고 해당 페이지의 이미지를 표시하는데 사용
...
function displayImg(page){
    const startIndex = page * imagesPerpage; // 현재 페이지('page')와 페이지당 이미지 수(imagesPerpage)를 곱해서 시작 인덱스를 계산 페이지 내에서 표시할 이미지에 첫 번째 이미지 인데스를 나타냄 
    const endIndex = startIndex  +  imagesPerpage; //시작 인데스에 페이지가 이미지 수를 더해서 종료 인덱스를 계산 페이지 내에서 표시할 마지막 이미지에 다음 인덱스를 나타냄
    
    //시작 인덱스에 페이지당 이미지 수를 더해서 종료 인덱스를 계산
    //페이지 내에서 표시할 이미지의 마지막 이미지 다음 인덱스를 표시
    //이미지 배열('images')에서 시작 인덱스의 종료 인덱스 사이에 이미지를 추출해서 
    //pageImages 배열에 저장 이 배열은 현재 페이지에 표시될 이미지를 담고있음
...
function updateButtons() {
    ...
    /*
     ...
     첫번째 페이지 일 경우에는 이전 버튼을 비활성화 시켜서 첫 번째 페이지 일 경우에는 이전버튼을 클릭하지 못하게 막는다.


        ( currentPage + 1 ) * imageContainer = 다음 페이지에 해당하는 이미지의 첫 번째 인덱스를 나타냄
                              images.length = images 배열의 길이로, 페이지를 넘어갈 수 있는 이미지의 전체 수를 나타냄
        nextButton.disabled = ( currentPage + 1 ) * imageContainer >= images.length; : 다음 페이지에 해당하는 이미지가
        이미지 배열을 벗어나는지 확인
    ...
    */
    ...
    //다음 버튼에서 마지막 페이지일 경우 이미지가 없을 때 더이상 다음 버튼이 눌리는 것을 방지하기 위해서 다음 버튼 
    // 또한 클릭하지 못하게 막음 
...
prevButton.addEventListener('click', () => {
    if(currentPage > 0){ // 현재 페이지 번호가 0보다 클 경우에만 실행
        currentPage --; // 현재 페이지 번호에서 하나를 감소
...
/*
 ...
 imagesPrepage : 한 페이지당 표시되는 이미지의 수
 lmages.length : images 배열의 총 이미지 수 
...
nextButton.addEventListener('click', ()=> {
    if((currentPage + 1) *imagesPerpage < images.length){
        currentPage ++; // 현재 페이지의 번호를 하나 이동시켜 다음 페이지로 이동하게 함 
